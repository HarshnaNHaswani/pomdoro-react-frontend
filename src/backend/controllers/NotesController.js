...
export const completeNoteHandler = function (schema, request) {
  const user = requiresAuth.call(this, request);
  try {
    if (!user) {
      new Response(
    ...
    const { noteId } = request.params;
    const trashedNote = user.notes.find((note) => note._id === noteId);
    user.notes = user.notes.filter((note) => note._id !== noteId);
    user.trash.push({ ...trashedNote, completed: true });
    this.db.users.update({ _id: user._id }, user);
    return new Response(201, {}, { trash: user.trash, notes: user.notes });
  } catch (error) {
    return new Response(
